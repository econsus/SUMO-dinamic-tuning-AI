...
<routes>
    <vType id="DEFAULT_VEHTYPE" speedFactor="{sf}"/>
...
class SFSumoEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        sf = self.current_params["speedFactor"] + delta["speedFactor"]
        self.current_params["speedFactor"] = max(0.0, min(2.0, sf))

        self._stop_sumo()
        row = self._current_row
        self._start_sumo(int(row[0]), int(row[1]), int(row[2]), int(row[3]),
                         self.current_params["speedFactor"])
        self._run_warmup()
